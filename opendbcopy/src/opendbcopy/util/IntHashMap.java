 ...
 */
  ...
  private void construct(int initialCapacity, float loadFactor) {
    if (initialCapacity < 0) throw new IllegalArgumentException("Invalid initial capacity: " + initialCapacity);
    if (initialCapacity < DEFAULT_CAPACITY) initialCapacity = DEFAULT_CAPACITY;
    if (initialCapacity > MAXIMUM_CAPACITY) initialCapacity = MAXIMUM_CAPACITY;
    if (loadFactor <= 0.0f || Float.isNaN(loadFactor)) throw new IllegalArgumentException("Invalid load factor: " + loadFactor);
